...
test:test("doesn't allow constructing span without a name", function(test)
	...
	test:ok(not ok)
	test:ok(err:endswith('name should be a string'))
end)
...
test:test("doesn't allow invalid child_of", function(test)
	...
	test:ok(not ok)
	test:ok(err:endswith('child_of should be a span or span context'))
end)
...
test:test("doesn't allow invalid references", function(test)
	...
	test:ok(not ok)
	test:ok(err:endswith('references should be a table'))
end)
...
test:test("checks for known extractor", function(test)
	...
	test:ok(not ok)
	test:ok(err:endswith('Unknown format: my_unknown_type'))
end)
